...
const getPackages = (packageConfigs, rootPath) => {
  ...
  packageConfigs.forEach(globPath => {
    glob.sync(path.join(globPath, PACKAGE_JSON), globOpts).forEach(globResult => {
...
module.exports.jest = compose(mapValues(key => `^${key}$`), createAliases)

module.exports.rollup = createAliases

module.exports.webpack = compose(mapValues(key => `${key}$`), createAliases)
